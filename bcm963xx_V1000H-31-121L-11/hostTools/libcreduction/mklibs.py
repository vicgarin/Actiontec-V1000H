...
# Find complete path of a /private, /public, or gpl library, by searching in lib_ppg_path
def find_ppg_lib(lib):
    for path in lib_ppg_path:
        if os.access(path + "/" + lib, os.F_OK):
            return path + "/" + lib
...
while 1:
    ...
    if passnr > 20:
        debug(DEBUG_NORMAL, "I: Maximum number of passes exceeded. Giving up.")
        break
...
#                 raise "Unresolvable symbol " + symbol
                 debug (DEBUG_VERBOSE, 'Ignoring unsolvable symbol', symbol)
    ...
    for library in libraries.elems():
        ...
        if re.match("^libpthread", library):
            has_libpthread = 1
            if passnr == 1:
                debug(DEBUG_NORMAL, "  i: This build includes libpthreads.")
...
#                  raise "No library provides non-weak " + symbol
                     soname = ''
                     debug (DEBUG_VERBOSE, 'Ignoring absent non-weak library symbol', symbol)
    ...
    for library in libraries.elems():
        ...
        if not pic_file:
            ...
            if soname == "":
                debug(DEBUG_NORMAL, so_file, " has no soname, copying")
                continue
	    ...
	    if using_ctor_dtor == 1:
            ...
            if passnr == 1:
                debug(DEBUG_NORMAL, "  i: reducing ", library)

            # When reducing libc.a, must provide libpthread.a on the link line.
            # Otherwise, compiler will select the weak pthread stubs inside libc.so,
            # which do not really provide locking and will cause malloc to fail
            # in multi-threaded builds.
            if has_libpthread and re.match("^libc[\.-]", library):
                extra_pic_file = find_pic("libpthread.so.0")
            else:
                extra_pic_file = ""
...
for lib in regexpfilter(os.listdir(dest_path), "(.*so[.\d]*)$").elems():
    ...
    if soname:
        ...
        # some of the wl shared libs do not have this field.  Does not seem
        # to break anything, but should be investigated
        debug(DEBUG_VERBOSE, "strange, no SONAME field in elf header for " + lib)

# BCM
# This script thinks all libraries are in /lib.  However, in CMS, we
# put libs in /lib, /lib/private, /lib/public, and maybe in the future
# /lib/gpl.  At this point, all libraries are in the uclibc subdir.
# We need to copy the reduced libraries back to the right place.
#
for lib in lib_path:
	if lib.find("fs.install/lib/private") > -1:
		lib_ppg_path.append(lib)
	if lib.find("fs.install/lib/public") > -1:
		lib_ppg_path.append(lib)
	if lib.find("fs.install/lib/gpl") > -1:
		lib_ppg_path.append(lib)

for lib in regexpfilter(os.listdir(dest_path), "(.*so[.\d]*)$").elems():
    ppg_path = find_ppg_lib(lib)
    if ppg_path:
        debug(DEBUG_VERBOSE, "copy up ppg_path=" + ppg_path)
        os.rename(dest_path + "/" + lib, ppg_path)
    else:
        debug(DEBUG_VERBOSE, "skipping " + lib + " .  Not in public or private or gpl")
