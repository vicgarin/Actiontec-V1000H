...
#define DIAG_TYPE_MASK 0x03000000
#define DIAG_TYPE_SHIFT 24
#define DIAG_PARTY_TYPE_SHIFT 4

#define	DIAG_DSL_CLIENT			0x0
#define	DIAG_XTM_CLIENT			0x1
#define	DIAG_WLAN_CLIENT		0x2
...
#if 0
extern void DiagsSetCommandHandler(unsigned long clientType, void *pCallback);
extern void *DiagsGetCommandHandler(unsigned long clientType);
#endif


extern dslDrvSkbPool * DevSkbAllocate(struct sk_buff *model,
		int skbMaxBufSize, int numOfSkbsInPool,
		int shortSkbMaxBufSize, int numOfShortSkbsInPool,
		int framePadSize, int frameHeaderLength);
extern struct sk_buff * GetSkb(dslDrvSkbPool *skbDev, int len);
extern int DevSkbFree(dslDrvSkbPool *skbDev, int enabeWA);
extern int DevSkbSendDiagsPacket(dslDrvSkbPool *skbDev, unsigned long cmd, dslDrvFragment *pFrag, int nFrag);
extern int DiagBuildFragTbl(dslDrvFragment *frag, char *p0, int len0, char *p1,  int len1);
