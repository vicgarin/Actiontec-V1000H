   ...
   PBCMXTMRT_DEV_CONTEXT pDevCtx = dev->priv;
    ...
    if( pDevCtx->ulLinkState == LINK_UP )
    {
        ...
        if( pDevCtx->ulTxQInfosSize )
        {
            ...
            if( pTqi->ulFreeBds && QueuePacket(pGi, pTqi, isAtmCell) )
            {
    ...
    if( ulStopRunning )
    {
             ...
             i++, pRxDma++ )
        {
            if( pGi->RxBdInfos[i].pBdBase )
            {
                PRXBDINFO pRxBdInfo = &pGi->RxBdInfos[i];
                volatile DmaStateRam  *pStRam;
                struct DmaDesc *pBdRead, *pBdWrite;
                UINT32 j, ulBdAddr;

                /* Disable receive interrupts. */
                pRxDma->cfg = 0;
                for( j = 0; j < 1000 &&
                    (pRxDma->cfg & DMA_ENABLE) == DMA_ENABLE; j++ )
                {
                    udelay(500);
                }
                if( (pRxDma->cfg & DMA_ENABLE) == DMA_ENABLE )
                    printk(KERN_ERR CARDNAME ":**** RX DMA STUCK ****\n");
                ...
                pStRam = pGi->pDmaRegs->stram.s;
                pStRam[SAR_RX_DMA_BASE_CHAN + i].state_data = 0;
                pStRam[SAR_RX_DMA_BASE_CHAN + i].desc_len_status = 0;
                pStRam[SAR_RX_DMA_BASE_CHAN + i].desc_base_bufptr = 0;

                /* Move all buffers to the beginning of the receive DMA ring.*/
                pBdRead = pBdWrite = (struct DmaDesc *) pRxBdInfo->pBdBase;
                for( ; ; )
                {
                    ulBdAddr = pBdRead->address;
                    pBdRead->length = pBdRead->address = 0;
                    pBdRead->status &= DMA_WRAP;
                    if( ulBdAddr )
                    {
                        if( (pBdWrite->status & DMA_WRAP) == DMA_WRAP )
                            pRxBdInfo->pBdTail = pRxBdInfo->pBdBase;
                        else
                            pRxBdInfo->pBdTail++;
                        pBdWrite->status |= DMA_OWN;
                        pBdWrite->length = RXBUF_SIZE;
                        pBdWrite->address = ulBdAddr;
                        pBdWrite++;
                    }

                    if( (pBdRead->status & DMA_WRAP) == DMA_WRAP )
                        break;
