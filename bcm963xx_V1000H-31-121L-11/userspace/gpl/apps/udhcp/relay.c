...
void process_relay_response(void)
{
    ...
    len = recvfrom(cur_relay->skt, buf, BUFFER_SIZE, 0, (struct sockaddr *)&from, &fromSize);
    if (len == -1) {
    ...
    for (iface = iface_config; iface; iface = iface->next) {
        if (from.sin_addr.s_addr == iface->relay_remote && ((struct dhcp *)buf)->giaddr == iface->server) {
