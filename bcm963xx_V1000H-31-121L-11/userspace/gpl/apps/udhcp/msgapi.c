...
void bcmExecOptCmd(struct optioncmd *optcmdnode, int option)
{
    ...
    for (iface = iface_config; iface; iface = iface->next) {
        for (i = 0; i < iface->max_leases; i++) {
            switch (option) {
            case 60:
                if (!strcmp(iface->leases[i].vendorid, optcmdnode->optionval)) {
                    bcmAddQosIP(optcmdnode, iface->leases[i].yiaddr, i);
                }
                break;
            case 61:
                ...
                break;
            case 77:
                if (!strcmp(iface->leases[i].classid, optcmdnode->optionval)) {
                    bcmAddQosIP(optcmdnode, iface->leases[i].yiaddr, i);
                }
                break;
            default:
