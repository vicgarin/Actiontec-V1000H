...
public class Basic
{
    ...
    private StringBuffer getlb()
    {
        ...
        if (found.size() > 0)
        {
            Bundle[] bundles = (Bundle[]) found.toArray(new Bundle[found.size()]);
            
            // Display active start level.
            if (sl != null)
            {
                  lb.append("LB:START LEVEL:" + sl.getStartLevel());
            }
            else
                  lb.append("LB:START LEVEL:-1");
            ...
            for (Bundle bundle : bundles)
            {
               ...
               if (name == null)
               {
                  temp = bundle.getLocation();
                  // location will be in the form file:location_name, we need to get rid of "file:"
                  words = temp.split(":");
                  //words[0] contains file, words [1] location, dropping :
                  name = words[1];
               }
               ...
               else
               {
                   lb.append(
                       String.format(
                           ":%d | %s | %d | %s",
                           bundle.getBundleId(), getStateStringwithoutBlank(bundle),
    ...
    private static String getStateStringwithoutBlank(Bundle bundle)
    {
        ...
        @Descriptor("target URLs") String[] urls)
    {
        ...
        for (String url : urls)
        {
            ...
            catch (IllegalStateException ex)
            {
                ...
                System.out.println("install:Bundle ID:"+location+":ERR:" + ex.toString());
            }
            catch (BundleException ex)
            {
                if (ex.getNestedException() != null)
                {
                    ...
                    System.out.println("install:Bundle ID:"+location+":ERR:" + ex.getNestedException().toString());
                }
                else
                {
                    //System.err.println(ex.toString());
        ...
        StringBuffer lb = getlb();
        
        
        //if (sb.toString().indexOf(',') > 0)
        //{
        //    System.out.println("install:Bundle IDs:" + sb.toString() + ":" +lb.toString());
        //}
        //else 
        if (sb.length() > 0)
        {
            System.out.println("install:Bundle ID:" + sb.toString() + ":" +lb.toString());
        }
        else
        {
            if(urls.length > 0)
               System.out.println("install:Bundle ID:"+urls[0]+":ERR:Internal Error.");
            else
               System.out.println("install:Bundle ID:(null):ERR:Internal Error,url is null.");
        }
    }
        ...
        @Descriptor("target bundle identifiers or URLs") String[] ss)
    {
        ...
        if ((ss != null) && (ss.length >= 1))
        {
            for (String s : ss)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        System.out.println("start:Bundle ID:"+id+":ERR:Bundle ID " + id + " is invalid.");
                    }
                }
                catch (NumberFormatException ex)
                {
                    //System.err.println("Unable to parse id '" + id + "'.");
                    System.out.println("start:Bundle ID:"+id+":ERR:Unable to parse id '" + id + "'.");
                }
                catch (BundleException ex)
                {
                    if (ex.getNestedException() != null)
                    {
                        ex.printStackTrace();
                        //System.err.println(ex.getNestedException().toString());
                        System.out.println("start:Bundle ID:"+id+":ERR:" + ex.getNestedException().toString());
                    }
                    else
                    {
                        //System.err.println(ex.toString());
                        System.out.println("start:Bundle ID:"+id+":ERR:" + ex.toString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if(sb.length() > 0)
               System.out.println("start:Bundle ID:"+sb.toString()+":"+ lb.toString());
            else
               System.out.println("start:Bundle ID:(null):ERR:sb.length()==0.");
        }
        else
        {
            //System.err.println("Incorrect number of arguments");
            System.out.println("start:Bundle ID:(null):ERR:Incorrect number of arguments");
        }
    }
        ...
        @Descriptor("target bundles") Bundle[] bundles)
    {
        ...
        if ((bundles == null) || (bundles.length == 0))
        {
            ...
            System.out.println("stop:Bundle ID:(null):ERR:Please specify the bundles to stop.");
        }
        ...
        {            
            ...
            catch (BundleException ex)
            {
                ...
                {
                    //System.err.println(ex.getNestedException().toString());
                    System.out.println("stop:Bundle ID:"+id+":ERR:" + ex.getNestedException().toString());
                }
                else
                {
                    //System.err.println(ex.toString());
                    System.out.println("stop:Bundle ID:"+id+":ERR:" + ex.toString());
                }
            }
            catch (Exception ex)
            {
        ...
        if(sb.length() > 0)
            System.out.println("stop:Bundle ID:"+sb.toString()+":" + lb.toString());
        else
            System.out.println("stop:Bundle ID:(null):ERR:sb.length()==0.");
    }
        ...
        @Descriptor("target bundles") Bundle[] bundles)
    {
        ...
        if ((bundles == null) || (bundles.length == 0))
        {
            ...
            System.out.println("uninstall:ERR:Please specify the bundles to uninstall.");
        }
        else
        {
            try
            {
                ...
                //System.err.println("uninstall--getlb");
                StringBuffer lb = getlb();

                if(sb.length() > 0)
                  System.out.println("uninstall:Bundle ID:"+sb.toString()+":" + lb.toString());
                else
                  System.out.println("uninstall:Bundle ID:(null):ERR:sb.length()==0.");
                //System.err.println("uninstall--ok");
            }
            catch (BundleException ex)
            {
                if (ex.getNestedException() != null)
                {
                    ...
                    //System.err.println(ex.getNestedException().toString());
                    if(sb.length() > 0)
                     System.out.println("uninstall:Bundle ID:"+sb.toString()+":ERR:" + ex.getNestedException().toString());
                    else
                     System.out.println("uninstall:Bundle ID:(null):ERR:sb.length()==0.");
                }
                else
                {
                    //System.err.println(ex.toString());
                    if(sb.length() > 0)
                     System.out.println("uninstall:Bundle ID:"+sb.toString()+":ERR:" + ex.toString());
                    else
                     System.out.println("uninstall:Bundle ID:(null):ERR:sb.length()==0.");
                }
            }
        ...
        @Descriptor("target bundle") Bundle bundle)
    {
        ...
        if(bundle != null)
        {
            id = String.valueOf(bundle.getBundleId());
        }
        else
        {
            id = "(null)";
        ...
        try
        {
            ...
            if (bundle != null)
            {
                ...
                System.out.println("update:Bundle ID:"+id+":"+ lb.toString());
            }
            else
            {
                System.out.println("update:Bundle ID:"+id+":ERR:Please specify a bundle to update");
            }
        }
        catch (BundleException ex)
        {
            ...
            {
                //System.err.println(ex.getNestedException().toString());
                System.out.println("update:Bundle ID:"+id+":ERR:" + ex.getNestedException().toString());
            }
            else
            {
                //System.err.println(ex.toString());
                System.out.println("update:Bundle ID:"+id+":ERR:" + ex.toString());
            }
        }
        catch (Exception ex)
        {
        ...
        @Descriptor("URL from where to retrieve bundle") String location)
    {
        ...
        if (location != null)
        {
            ...
            catch (MalformedURLException ex)
            {
                ...
                System.out.println("update:Bundle ID:"+id+":ERR:Unable to parse URL");
            }
            catch (IOException ex)
            {
                //System.err.println("Unable to open input stream: " + ex);
                System.out.println("update:Bundle ID:"+id+":ERR:Unable to open input stream:" + ex);
            }
            catch (BundleException ex)
            {
                if (ex.getNestedException() != null)
        ...
        else
        {
            ...
            System.out.println("update:Bundle ID:"+id+":ERR:Must specify a location.");
        }
    }
        ...
        boolean showSymbolic, boolean showUpdate)
    {
        ...
        if (startLevel != null)
        {
            ...
            lb.append("LB:START LEVEL:" + startLevel.getStartLevel());
        }
        else
            lb.append("LB:START LEVEL:-1");
