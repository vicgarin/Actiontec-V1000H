...
int main(int argc, char **argv, char **envp)
{
    ...
    volatile int launchpppd = 1, debug = 1;
    //volatile int launchpppd = 1, debug = 0;
    int opt;

    while ((opt = getopt(argc, argv, "r:U:P:C")) != -1) {
	switch (opt) {
// brcm
#if 1
		case 'r':
...
#if 1
    launchpppd = 1;
    inetaddr = get_ip_address(iremote);
#endif
...
void launch_pppd(char *ttydev, int argc, char **argv)
{
    char *new_argv[12];/* XXX if not using GCC, hard code a limit here. */
...
// brcm
    ...
    if (strlen(iuserid)) {
	new_argv[arg++] = "-u";
	new_argv[arg++] = strdup(iuserid);
    }
    if (strlen(ipassword)) {
	new_argv[arg++] = "-p";
	new_argv[arg++] = strdup(ipassword);
    }
    new_argv[arg++] = "-T";
    new_argv[arg++] = strdup(ttydev);
