...
static void dfree(struct dnode **dnp, struct dnode *orig_dnode0)
{
	...
	// BRCM fix.  The problem is the original element at index 0
	// may have been moved to the middle of the array due to the
	// sorting.  Must start freeing from the original head element
	// so that all elements in the linked list are freed.
	if (orig_dnode0 == NULL) {
		printf("WARNING: dfree got NULL orig_dnode0\n");
		return;
	}
	cur = orig_dnode0;
...
static void showdirs(struct dnode **dn, int ndirs, int first)
{
	...
	for (i = 0; i < ndirs; i++) {
		...
		if (nfiles > 0) {
			...
			dfree(subdnp, orig_dnode0);	/* free the dnodes and the fullname mem */
#endif
		}
	}
}
