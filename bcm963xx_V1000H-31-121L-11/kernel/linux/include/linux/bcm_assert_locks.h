...
#include <linux/smp.h>
#define     HAS_SPINLOCK_COND(s)  (s->owner == current && s->owner_cpu == smp_processor_id())
#define NOT_HAS_SPINLOCK_COND(s)  (s->owner != current && s->owner_cpu != smp_processor_id())
#else
...
#define     HAS_MUTEX_COND(m)     (m->owner == current_thread_info())
#define NOT_HAS_MUTEX_COND(m)     (m->owner != current_thread_info())
#else
