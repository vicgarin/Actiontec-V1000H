...
static void ndisc_router_discovery(struct sk_buff *skb)
{
	...
	if (!(ipv6_addr_type(&ipv6_hdr(skb)->saddr) & IPV6_ADDR_LINKLOCAL)) {
        ...
        printk(KERN_ERR "ICMPv6 RA: source address is not link-local.\n");
		return;
	}
	if (optlen < 0) {
		ND_PRINTK2(KERN_WARNING
			   "ICMPv6 RA: packet too short\n");
        printk(KERN_ERR "ICMPv6 RA: packet too short\n");
		return;
	}
	...
	if (skb->ndisc_nodetype == NDISC_NODETYPE_HOST) {
        ...
        printk(KERN_ERR "ICMPv6 RA: from host or unauthorized router\n");
		return;
	}
#endif
	...
	if (in6_dev == NULL) {
		ND_PRINTK0(KERN_ERR
			   ...
			   skb->dev->name);
        printk(KERN_ERR "ICMPv6 RA: can't find inet6 device for %s.\n",skb->dev->name);
		return;
	}
#if defined(CONFIG_MIPS_BRCM)
	...
	if (!ndisc_parse_options(opt, optlen, &ndopts)) {
        ...
        printk(KERN_ERR "ICMP6 RA: invalid ND options\n");
		return;
	}
