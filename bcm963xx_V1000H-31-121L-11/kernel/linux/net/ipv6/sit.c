...
static int ipip6_tunnel_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (mtu > IPV6_MIN_MTU)
	    {
	        /*
	        * for CDRouter test ipv6_6rd_13(DF flag not set)
	        * if the pkg true size is smaller than IPV6_MIN_MTU,set DF to 0
	        */
	        //printk(KERN_ERR "ipip6_tunnel_xmit: skb->len=%d,IPV6_MIN_MTU=%d\n",skb->len,IPV6_MIN_MTU);
	        if(skb->len -sizeof(struct iphdr) <= IPV6_MIN_MTU)
                iph->frag_off	=	0;
            else
