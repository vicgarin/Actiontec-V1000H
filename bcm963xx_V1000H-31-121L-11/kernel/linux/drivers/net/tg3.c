...
#define DRV_MODULE_VERSION	\
	__stringify(TG3_MAJ_NUM) "." __stringify(TG3_MIN_NUM) "j"
#define DRV_MODULE_RELDATE	"January 21, 2011"
...
/* see pr141646 */
#define TG3_FW_UPDATE_TIMEOUT_SEC	10
#else
...
static void tg3_mdio_start(struct tg3 *tp)
{
	...
	/* FIXME -- This shouldn't be required, but without it, the device
	 *          will not pass traffic until the phy is reset via a link
	 *          up event or through a change in speed settings.
	 */
	val = MII_TG3_AUXCTL_SHDWSEL_MISC |
	      (MII_TG3_AUXCTL_SHDWSEL_MISC << 12);
	...
	if (tp->link_config.autoneg == AUTONEG_ENABLE &&
		...
		case TG3_CL45_D7_EEERES_STAT_LP_1000T:
			if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5717 ||
				...
				tg3_phydsp_write(tp, 0x1a, 0x0000);
...
out:
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5785 &&
		...
		val = MII_TG3_MISC_SHDW_RGMII_SEL;
		tg3_writephy(tp, MII_TG3_MISC_SHDW, val);
		tg3_readphy(tp, MII_TG3_MISC_SHDW, &val);
		val &= ~(MII_TG3_MISC_SHDW_RGMII_MODESEL0 |
			 MII_TG3_MISC_SHDW_RGMII_MODESEL1);
		val |= MII_TG3_MISC_SHDW_WREN;
		tg3_writephy(tp, MII_TG3_MISC_SHDW, val);
	}
...
static void tg3_phy_copper_begin(struct tg3 *tp)
{
	...
	if (tp->phy_flags & TG3_PHYFLG_EEE_CAP) {
		...
		if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5717 ||
			...
			tg3_phydsp_write(tp, 0x1a, 0x0007);
		}
...
#ifdef TG3_VMWARE_NETQ_ENABLE
		tnapi->netq.stats.tx_packets++;
		tnapi->netq.stats.tx_bytes += skb->len;
#endif
...
#ifdef TG3_VMWARE_NETQ_ENABLE
	if ((tnapi->netq.flags & TG3_NETQ_TXQ_FREE_STATE) &&
	    sw_idx == tnapi->tx_prod)
		wake_up(&tp->vmware.netq.tx_wait);
#endif
...
static int tg3_rx(struct tg3_napi *tnapi, int budget)
{
	...
	while (sw_idx != hw_idx && budget > 0) {
		...
		tnapi->netq.stats.rx_packets++;
		tnapi->netq.stats.rx_bytes += len;
#endif
...
static int tg3_poll_msix(struct napi_struct *napi, int budget)
{
	...
	while (1) {
		...
		if (likely(sblk->idx[0].tx_consumer == tnapi->tx_cons &&
			   *(tnapi->rx_rcb_prod_idx) == tnapi->rx_rcb_ptr)) {
#ifdef TG3_VMWARE_NETQ_ENABLE
			if (tnapi->netq.flags & TG3_NETQ_RXQ_FREE_STATE)
				wake_up(&tp->vmware.netq.rx_wait);
#endif
...
#ifdef TG3_VMWARE_NETQ_ENABLE
		if (!i || (i && (tp->tg3_flags3 & TG3_FLG3_ENABLE_RSS)))
#endif
...
#ifdef TG3_VMWARE_NETQ_ENABLE
	val = tp->grc_mode | GRC_MODE_IRQ_ON_MAC_ATTN | GRC_MODE_HOST_STACKUP;
...
#ifdef TG3_VMWARE_NETQ_ENABLE
	if ((tp->tg3_flags3 & TG3_FLG3_IOV_CAPABLE) &&
	    tg3_netq_force[tp->vmware.netq.index] > 0)
		tp->irq_cnt = tg3_netq_force[tp->vmware.netq.index] + 1;
#endif
...
#ifdef TG3_VMWARE_NETQ_ENABLE
		if (tp->tg3_flags3 & TG3_FLG3_IOV_CAPABLE)
			tp->tg3_flags3 |= TG3_FLG3_ENABLE_IOV;
		else
#endif
...
{
#if TG3_VLAN_TAG_USED && defined(BCM_HAS_VLAN_FEATURES)
...
static int __devinit tg3_get_invariants(struct tg3 *tp)
{
	...
	if (tp->pci_chip_rev_id == CHIPREV_ID_5700_B0)
		...
		unsigned long features = NETIF_F_SG | NETIF_F_GRO;
	...
	if (tp->tg3_flags2 & TG3_FLG2_5750_PLUS) {
		...
		if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_5718 ||
			...
			tp->tg3_flags3 |= TG3_FLG3_IOV_CAPABLE;
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5701 &&
		...
		tp->rx_offset = ALIGN(tp->rx_offset - NET_IP_ALIGN, 8);
				  ...
				  const struct pci_device_id *ent)
{
	...
	netdev_info(dev, "Jumbo Frames capable[%d]\n",
		    (tp->tg3_flags & TG3_FLAG_JUMBO_CAPABLE) != 0);
#ifdef TG3_VMWARE_NETQ_ENABLE
	if (tp->tg3_flags3 & TG3_FLG3_IOV_CAPABLE)
